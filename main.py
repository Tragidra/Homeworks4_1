...
class Solver:
	...
	def setupTablue(self):
		...
		if len(self.A) - noEqualityConst > 0:
			...
			for row in matrix:
				row[len(self.x):-1] = []
			self.steps.append({"step":self.step,"text":"Dobavili rezervnie peremennie", "s": [("s%i" % i) for i in range(1, len(self.A) - noEqualityConst + 1)], "type":0, "matrix": matrix})
			self.step += 1

		self.x.extend([3 for i in range(len(self.artificial))])

		if noGreaterThanConst + noEqualityConst > 0:
			self.steps.append({"step":self.step,"text":"Dobavili isskustv peremennie", "s": [("a%i" % i) for i in range(1, noGreaterThanConst + noEqualityConst + 1)], "type":0, "matrix": self.getTablue()})
			self.step += 1
	...
	def getJSONsolution(self):
		...
		solution["Da/Net"] = self.success
		solution["Kol"] = self.steps
		jsonSolution = {"Otvet": solution}
...
if __name__ == "__main__":
	...
	try:
		jsonString = '{"c":[1,1],"x":[0,0],"A":[[4,-4],[4,-1],[-2,4]],"b":[7,8,7],"e":[0,0,1],"type":1}'
		#Тут менять, e - знаки, a - кэфы, b - числа после знаков, c - кэфы в главном уравнении
	except:
